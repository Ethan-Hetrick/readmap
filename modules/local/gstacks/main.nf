process STACKS_REFMAP {
    ...
    container "${ workflow.containerEngine in ['singularity', 'apptainer'] && !task.ext.singularity_pull_docker_container ?
        'https://depot.galaxyproject.org/singularity/stacks:2.68--h077b44d_3':
        'quay.io/biocontainers/stacks:2.68--h077b44d_3' }"
    ...
    tuple val(meta), path("stacks.run_config.txt")            , emit: run_config
    ...
    script:
    def gstacks_args = task.ext.gstacks_args ?: ''
    def populations_args = task.ext.populations_args ?: '--vcf --ordered-export -p 1 -r 0.65 --min-mac 3 --max-obs-het 0.6 --write-random-snp'
    """
    cat <<-END_CONFIG > stacks.run_config.txt
    pipeline_mode: direct_gstacks_populations
    sample_suffix: processed
    gstacks_args: ${gstacks_args}
    populations_args: ${populations_args}
    END_CONFIG
    ...
    printf '%s\\n' 'Direct gstacks + populations execution; ref_map.pl not used.' > ref_map.log
    """
